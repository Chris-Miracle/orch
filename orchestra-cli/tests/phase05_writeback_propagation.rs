...
#[test]
fn phase05_writeback_end_to_end_propagation() {
    ...
    codebase.projects[0].tasks.push(Task {
        ...
        title: "Task A in progress".to_owned(),
        status: TaskStatus::InProgress,
        description: Some("should be auto-completed by writeback".to_owned()),
        subtasks: vec![],
        notes: vec![],
        created_at: now,
        updated_at: now,
    ...
    registry::save_codebase_at(home.path(), &project, &codebase).expect("save codebase with task");
    ...
    // Let own-write suppression window clear before external edit.
    sleep(Duration::from_millis(1200));

    let original = std::fs::read_to_string(&claude).expect("read CLAUDE.md");
    let with_block = format!(
        "{original}\n<!-- orchestra:update -->\ntask_completed: task-A\n<!-- /orchestra:update -->\n"
    );
    std::fs::write(&claude, with_block).expect("append update block");

    let propagated = wait_until(Duration::from_secs(8), || {
        let codebase = match registry::load_codebase_at(home.path(), &project, &codebase_name) {
            Ok(cb) => cb,
            Err(_) => return false,
        };
        let done = codebase.projects[0]
            .tasks
            .iter()
        ...
        let claude_ok = std::fs::read_to_string(&claude)
            .map(|text| !text.contains("orchestra:update"))
            .unwrap_or(false);

        let cursor_ok = std::fs::read_to_string(&cursor)
            .map(|text| !text.contains("Task A in progress") && !text.contains("task-A"))
            .unwrap_or(false);
        ...
        let log_ok = std::fs::read_to_string(events_path)
            .map(|text| text.contains("task_completed: task-A"))
            .unwrap_or(false);

        done && claude_ok && cursor_ok && log_ok
    });

    assert!(
        propagated,
        "writeback propagation did not complete within timeout"
    );
